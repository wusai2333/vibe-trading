...
ALL = STABLE + ["limit_dist", "vol_ivol60"]
...
VARIANTS = {"stable5": STABLE,
            "plus_limit_dist": STABLE + ["limit_dist"],
            "plus_dist_ivol": STABLE + ["limit_dist", "vol_ivol60"]}
out = {}
for name, ids in VARIANTS.items():
    ...
    wstats = {}
    for extra in ("limit_dist", "vol_ivol60"):
...
json.dump(out, open(DATA / "csi300_limit_vol_test.json", "w"), ensure_ascii=False, indent=1)
...
print(f"SAVED {DATA / 'csi300_limit_vol_test.json'}", file=sys.stderr)
