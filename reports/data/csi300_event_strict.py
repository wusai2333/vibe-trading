"""Strict bench for the event zoo (earnings preview/express factors).

Same protocol: random control (5 seeds) + OOS split. Injects event:* columns.
"""
...
print("panel enriched: " + str(sum(1 for k in panel if k.startswith('event:'))) + " event columns", file=sys.stderr)
...
t0 = time.time()
r = run_bench_strict(zoo="event", universe="csi300-ashare",
...
json.dump(rows, open(DATA / "csi300_event_strict.json", "w"),
          ensure_ascii=False, indent=1, default=str)
print("SAVED csi300_event_strict.json", file=sys.stderr)
