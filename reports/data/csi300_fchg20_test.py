"""Correlation gate + constrained incremental test for margin_fchg20.
...
_margin = pickle.load(open(DATA / "margin_panel.pkl", "rb"))
for _k, _v in _margin.items():
    panel[f"margin:{_k}"] = _v
...
rev5 = zscore(reg.compute("margin_fchg20", panel).rolling(10, min_periods=6).mean())
...
if max_abs >= CORR_GATE:
    json.dump(result, open(DATA / "csi300_fchg20_test.json", "w"), ensure_ascii=False, indent=1)
    sys.exit(0)
