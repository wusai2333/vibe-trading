"""Correlation gate + constrained incremental test for pool2_vpcorr20.
...
rev5 = zscore(reg.compute("pool2_vpcorr20", panel).rolling(10, min_periods=6).mean())
...
if max_abs >= CORR_GATE:
    json.dump(result, open(DATA / "csi300_vpcorr20_test.json", "w"), ensure_ascii=False, indent=1)
    sys.exit(0)
