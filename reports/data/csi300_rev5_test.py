"""Correlation gate + constrained incremental test for pool2_rev5.
...
rev5 = zscore(reg.compute("pool2_rev5", panel).rolling(10, min_periods=6).mean())
...
if max_abs >= CORR_GATE:
    json.dump(result, open(DATA / "csi300_rev5_test.json", "w"), ensure_ascii=False, indent=1)
    sys.exit(0)
