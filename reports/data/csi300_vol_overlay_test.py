"""降险层实测（2026-08-24，用户拍板跑方案 1）：8 月式反转期能否少亏？

预注册判据（跑之前定死，防数据窥探）：
  PASS 需同时满足——
  a) 2026-08 子段净收益较基线改善 >= 1.5pp（约把亏损砍半）
  b) 全期 OOS(2019-) Sharpe >= 基线 - 0.10
  c) 剔牛市(去 2024-25) Sharpe >= 基线 - 0.10
  只改善 8 月而全期明显劣化 -> 拒（D 系列）。
参数网格固定为整数档，不看结果调参；2025-08（另一个历史最差 8 月）
作第二案例交叉验证，防单月过拟合。

变体：
  none        对照（生产口径 top15/5d/10bps）
  vt15/20/25  指数波动率目标：exposure = min(1, T/sigma_idx_20d_ann)
  ma20/ma60   指数收盘 < MA_N -> 半仓，否则满仓
  disp80      截面收益离散度 20d 均值 > 滚动252 80分位 -> 半仓（反转regime探测）
...
def main():
    ...
    for T in (0.15, 0.20, 0.25):
        exposures[f"vt{int(T*100)}"] = (T / idx_vol20).clip(upper=1.0).fillna(1.0)
    for N in (20, 60):
        exposures[f"ma{N}"] = pd.Series(np.where(idx > idx.rolling(N).mean(), 1.0, 0.5),
                                        index=days)
    exposures["disp80"] = pd.Series(np.where(disp_flag, 0.5, 1.0), index=days)
    ...
    line("none", base, bm)
    for name in ["vt15", "vt20", "vt25", "ma20", "ma60", "disp80"]:
        net = backtest(exposures[name])
        line(name, net, metrics(net))

    # 8 月内最痛 5 日回撤对比（2026-08）
    print("\n2026-08 段内最大回撤（净值）：")
    for name in ["none", "vt20", "ma20", "disp80"]:
