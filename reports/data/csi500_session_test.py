...
ALL = STABLE + ["session_onin20", "session_on20", "session_on5"]
print("computing factors...", file=sys.stderr)
_cache = pickle.load(open(DATA / "csi500_factor_cache.pkl", "rb"))
...
VARIANTS = {"plus_onin20": STABLE + ["session_onin20"],
            "plus_onin20_on20": STABLE + ["session_onin20", "session_on20"]}
out = {}
for name, ids in VARIANTS.items():
    ...
    wstats = {}
    for extra in ("session_onin20", "session_on20"):
...
json.dump(out, open(DATA / "csi500_session_test.json", "w"), ensure_ascii=False, indent=1)
...
print(f"SAVED {DATA / 'csi500_session_test.json'}", file=sys.stderr)
