...
class DataLoader:
    ...
    def _fetch_tencent(
        ...
        # Tencent drops the latest (in-progress) bar when an explicit end date
        # is given; leave end open when fetching through today so the latest
        # bar is included (_finalize slices back to end_date).
        cursor_end = "" if pd.Timestamp(end_date) >= pd.Timestamp.today().normalize() else end_date
