...
def _yahoo_candidate(quote: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    Yahoo carries US tickers bare, HK tickers as ``0700.HK``, A-shares as
    ``600519.SS`` / ``000858.SZ``, and Canadian listings with ``.TO`` / ``.V``
    suffixes. We translate those into the project convention (``AAPL.US`` /
    ``00700.HK`` / ``600519.SH`` / ``000858.SZ`` / ``TD.TO`` / ``PNG.V``) and
    leave other instruments (crypto, indices, FX) on their native Yahoo symbol.
...
def _from_yahoo_symbol(raw_symbol: str, quote: Dict[str, Any]) -> tuple[str, str]:
    ...
    # Yahoo tags A-shares with exchange suffixes (.SS Shanghai, .SZ Shenzhen)
    # that differ from the project convention (.SH/.SZ). Without this mapping
    # the same listing surfaces as TWO candidates (Eastmoney's 600519.SH plus
    # Yahoo's 600519.SS), the grounding gate sees an ambiguous identity and
    # blocks every A-share tool call with identity_conflict.
    if upper.endswith(".SS"):
        return f"{upper[: -len('.SS')]}.SH", "cn"
    if upper.endswith(".SZ"):
        return f"{upper[: -len('.SZ')]}.SZ", "cn"
