...
class TestAShareSymbolConvention:
    """Yahoo A-share suffixes (.SS/.SZ) must land in the project convention.

    Yahoo tags Shanghai listings as ``600519.SS`` and Shenzhen listings as
    ``000858.SZ``. Left untranslated, the same listing surfaces as two
    candidates (Eastmoney's ``600519.SH`` plus Yahoo's ``600519.SS``), the
    grounding gate sees an ambiguous identity and every A-share tool call is
    blocked with identity_conflict.
    """

    def test_yahoo_ss_and_sz_are_translated_and_merged(self):
        em = {
            "QuotationCodeTable": {
                "Data": [
                    {
        ...
        with patch.object(ss.eastmoney_client, "get_json", return_value=em), patch.object(
            ss.yahoo_client, "search", return_value=yahoo
        ):
            out = ss.SymbolSearchTool().execute(query="600519", limit=10)

        payload = json.loads(out)
        by_symbol = {c["symbol"]: c for c in payload["data"]["candidates"]}

        # No Yahoo-native A-share spelling may leak into the envelope.
        assert "600519.SS" not in by_symbol

        # Both listings land in the project convention, corroborated + merged.
        assert by_symbol["600519.SH"]["market"] == "cn"
        assert "yahoo" in by_symbol["600519.SH"].get("also_from", [])
        assert by_symbol["000858.SZ"]["market"] == "cn"
        assert "yahoo" in by_symbol["000858.SZ"].get("also_from", [])

    def test_yahoo_only_a_share_still_resolves_to_convention(self):
        """Eastmoney being down must not strand A-shares on Yahoo spellings."""
        yahoo = [
            {
                "symbol": "600519.SS",
                "shortname": "KWEICHOW MOUTAI",
        ...
        with patch.object(
            ...
            "get_json",
            side_effect=RuntimeError("HTTP 429 banned"),
        ), patch.object(ss.yahoo_client, "search", return_value=yahoo):
            out = ss.SymbolSearchTool().execute(query="600519")

        payload = json.loads(out)
        symbols = [c["symbol"] for c in payload["data"]["candidates"]]
        assert symbols == ["600519.SH"]
        assert payload["data"]["candidates"][0]["market"] == "cn"
