...
def test_dual_spelling_a_share_candidates_collapse_and_lock(tmp_path: Path) -> None:
    """The same listing under two spellings collapses onto one canonical lock.

    Documents the gate side of the A-share identity_conflict bug: before
    symbol_search_tool translated Yahoo's ``.SS`` suffix, one resolution
    returned ``600519.SH`` (Eastmoney) plus ``600519.SS`` (Yahoo) for the
    same stock. The ledger normalizes ``.SS`` onto the canonical ``.SH``,
    so the two rows describe one listing and lock it instead of blocking
    the run — defense in depth behind the tool-level translation.
    """
    ledger = GroundingLedger(run_dir=tmp_path, user_message="分析贵州茅台")
    payload = _resolver_payload(
        candidates=[
            {"symbol": "600519.SH", "name": "贵州茅台", "market": "cn", "source": "eastmoney"},
            {"symbol": "600519.SS", "name": "KWEICHOW MOUTAI", "market": "global", "source": "yahoo"},
        ],
        query="600519",
    )
    ledger.authorize_tool_call(
...
def test_merged_a_share_candidate_locks_and_authorizes_consumer(tmp_path: Path) -> None:
    """A corroborated single-convention candidate locks and unblocks A-share calls.

    The post-fix symbol_search_tool merges Yahoo's (translated) hit into the
    Eastmoney one, so the resolver returns a single ``600519.SH`` candidate
    carrying ``also_from`` corroboration.
    """
    ledger = GroundingLedger(run_dir=tmp_path, user_message="分析贵州茅台")
    payload = _resolver_payload(
        candidates=[
